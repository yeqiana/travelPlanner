...
- 第二轮 `dialogIntent=DETAIL_PLAN`。
- 每天上午、下午、晚上三段尽量包含时间、地点、交通建议、预计耗时和二次确认提示。
- 三段文本应符合 `09:00-11:30 地点：xxx；安排：xxx；交通：xxx；耗时：预计x小时；确认：xxx需二次确认。` 的兼容格式。
- 前端表格应能把三段文本拆成时间、地点、安排、交通和耗时，不应把整段文本全部堆在“安排”列。
- 上一轮目的地、天数、人数、预算等上下文不应丢失。
- 未覆盖城市只能输出候选点并标注“需二次确认”。
- 文案不应出现“核心景点游览”“自由活动”“轻松收尾”等空泛主行程。

### 不允许出现的错误行为

- 第二轮原样返回上一轮计划。
- 因“具体一点”重新丢失上一轮上下文。
- 把未经实时核实的开放、预约、票价、余票写成确定事实。
- 为本阶段新增不兼容的 `activities` 字段并要求前端强依赖。
...
- P6 三段行程文本包含时间、地点、交通、预计耗时和二次确认提示。
- P6 前端表格可拆分显示时间范围、地点、安排、交通和耗时。
- 后续补充回复应先显示本轮补充和主要变化，再展示完整行程卡片。
- 聊天页行程视图应展示行程卡片；对话视图应只展示 Markdown，不重复展示行程卡片和行程操作按钮。

## 11. 用例 9：后续补充差异回复与双模式视图
...
- 第二轮 AgentTravel 回复顶部应出现“已根据你的补充更新”。
- 回复中应显示“本轮补充”和“主要变化”，不能只像重新生成完整计划。
- 行程视图下仍展示完整行程卡片和底部操作按钮。
- 对话视图下只展示 Markdown 差异回复，不展示行程卡片和行程操作按钮。
- 两种视图切换不应触发重新请求，也不应丢失历史消息。
